...
import { Diagnostic, error, aggregateDiagnostics } from './diagnostics.js';
import {
 ...
 */
export function safeParseYaml(content: string, filePath: string): unknown {
  // File size check
 ...
 */
export function parsePluginIdentity(
  filePath: string,
  content: string
): { identity: PluginIdentity; diagnostics: Diagnostic[] } {
  ...
  try {
    ...
    return { identity: null as unknown as PluginIdentity, diagnostics };
  }
  ...
  const identity: PluginIdentity = {
    ...
    id: result.data.id as PluginIdentity['id'],
    upstream: {
      repository: result.data.upstream.repository as PluginIdentity['upstream']['repository'],
 ...
 */
export function parseVersionRecord(
  filePath: string,
  content: string
): { version: VersionRecord; diagnostics: Diagnostic[] } {
  const diagnostics: Diagnostic[] = [];
  ...
  try {
    ...
    return { version: null as unknown as VersionRecord, diagnostics };
  }
  ...
  const version = result.data as any as VersionRecord;
 ...
 */
export function loadPluginIdentity(pluginDir: string): {
 ...
 */
export function loadVersionRecords(
  versionsDir: string
): { versions: VersionRecord[]; diagnostics: Diagnostic[] } {
...
export function loadRegistry(
  registryRoot: string
): { plugins: Plugin[]; diagnostics: Diagnostic[] } {
 ...
 */
export function getPluginIdFromDir(pluginDir: string): string {
